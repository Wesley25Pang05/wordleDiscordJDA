...
public class Wordle extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
			...
			for (int i = 0; i < members.size(); i++) {
				if (Objects.requireNonNull(event.getAuthor()).getId().equals(members.get(i).user)) {
					wordle(event, i);
					is = true;
					break;
				}
			...
			if (!is && event.getMessage().getContentRaw().toLowerCase().indexOf("play wordle") == 0) {
				...
				MemberManagement theMember = new MemberManagement(word, Objects.requireNonNull(event.getAuthor()).getId());
				...
				theMember.setLastPlayed(event);
			} else {
				System.out.println("error");
			}
	}

	public void wordle(MessageReceivedEvent event, int user) {
		String message = event.getMessage().getContentRaw().toLowerCase();
		MemberManagement player = members.get(user);
		if (player.wordle && message.length() == 11 && message.indexOf("guess ") == 0) {
			...
			embed.setFooter("STRIKETHROUGH IS WRONG\nITALIC IS WRONG SPOT\nBOLD IS CORRECT +" +
					"\nLAST PLAYED: " + player.getLastPlayed());
			player.setLastPlayed(event);
			player.prev += "\n";
	...
	public String check(String guess, MemberManagement player) {
		...
		player.win = "";
		for (int i = 0; i < 5; i++) {
			int letterInWord = Character.getNumericValue(guess.charAt(i)) - 10;
			if (player.word.charAt(i) == guess.charAt(i)) {
				replacement.append("**").append(guess.charAt(i)).append("** ");
				player.win += guess.charAt(i);
				player.correct[i] = "" + guess.charAt(i);
				player.used[letterInWord] = String.valueOf(guess.charAt(i));
			}
